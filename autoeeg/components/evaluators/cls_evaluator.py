...
class EEGClassificationEvaluator(_BaseEvaluator):
    ...
    def __call__(self, config, **kwargs):
        ...
        raw_list = self.data_node.data[0] # List of mne.io.Raw (Each is one Subject)
        event_list = self.data_node.data[1] # List of event arrays (Each is one Subject)
        subject_ids = np.array(self.data_node.subject_ids) # Length == len(raw_list)

        # 1. Define Splitting Strategy
        if self.split_by == 'subject':
            # Simplified: Indices correspond directly to subjects in the list
        ...
        for train_idx, val_idx in iterator:
            if self.split_by == 'subject':
                t_raw = [raw_list[i] for i in train_idx]
                t_evt = [event_list[i] for i in train_idx]
                t_sub = [subject_ids[i] for i in train_idx]
                
                v_raw = [raw_list[i] for i in val_idx]
                v_evt = [event_list[i] for i in val_idx]
                v_sub = [subject_ids[i] for i in val_idx]
            else:
                def get_trial_data(indices):
                    # Group trials by subject index
                    sub_to_trials = {}
                    for i in indices:
                        s_idx = flat_events_meta[i]['sub_idx']
                        l_idx = flat_events_meta[i]['local_idx']
                        if s_idx not in sub_to_trials:
                            sub_to_trials[s_idx] = []
                        sub_to_trials[s_idx].append(l_idx)
                    
                    active_subs = sorted(list(sub_to_trials.keys()))
                    return [raw_list[i] for i in active_subs], \
                           [event_list[i][sub_to_trials[i]] for i in active_subs], \
                           [subject_ids[i] for i in active_subs]
